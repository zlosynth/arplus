...
pub struct KarplusStrong {
    sample_rate: f32,
    // TODO: Use constant, see if it would be faster.
    feedback: f32,
    resonance: f32,
    frequency: f32,
    contour: f32,
...
impl KarplusStrong {
    ...
    pub fn populate_add(
            ...
            .enumerate()
        {
            ...
            let new_noise_sample = if let Some(buffer_noise) = noise_buffer {
                ...
                offset_noise * self.noise_envelope.pop() * self.pluck
            };
            ...
            let delayed_sample = {
                ...
                self.buffer.peek_interpolated(relative_index)
            };

            let mixed_sample = new_noise_sample + delayed_sample * self.feedback;
    ...
    // to intialize the structure, without having to pass randomizer there.
    fn trigger_without_random(
        &mut self,
        feedback: f32,
        frequency: f32,
        contour: f32,
